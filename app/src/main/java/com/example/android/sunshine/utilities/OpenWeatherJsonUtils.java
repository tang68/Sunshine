...
//        long normalizedUtcStartDay = SunshineDateUtils.normalizeDate(now);
        ...
        for (int i = 0; i < jsonWeatherArray.length(); i++) {
            ...
            JSONObject mainDayForeCast = dayForecast.getJSONObject(OWM_MAIN);
            JSONObject windDayForeCast = dayForecast.getJSONObject(OWM_WIND);
            JSONObject weatherObject = dayForecast.getJSONArray(OWM_WEATHER).getJSONObject(0);
            //JSONObject temperatureObject = dayForecast.getJSONObject(OWM_TEMPERATURE);

            dateTimeMillis = normalizedUtcStartDay + SunshineDateUtils.DAY_IN_MILLIS * i;
            pressure = mainDayForeCast.getDouble(OWM_PRESSURE);
            humidity = mainDayForeCast.getInt(OWM_HUMIDITY);
            windSpeed = windDayForeCast.getDouble(OWM_WINDSPEED);
            windDirection = windDayForeCast.getDouble(OWM_WIND_DIRECTION);
            weatherId = weatherObject.getInt(OWM_WEATHER_ID);
            high = mainDayForeCast.getDouble(OWM_MAX);
            low = mainDayForeCast.getDouble(OWM_MIN);
